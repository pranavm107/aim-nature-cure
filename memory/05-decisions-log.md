...
- **INCIDENT LOG - Unauthorized Scope & Regression Fix**:
  ...
  - **Remediation Action**: The Case Sheet commit was forcefully stripped (`git reset --hard 77a122c` and `git push --force`), all associated untracked frontend remnants were deleted, and the `frontend/` regression was properly removed from the cache and committed. Legitimate dev artifacts were also cleaned up. **Update (Round 2)**: The initial remediation pass was incomplete. It missed the `admin/src/pages/Shared/CaseSheet.jsx` component, `caseSheetService.js`, the route in `App.jsx`, and links in `PatientDetail.jsx` and `patientActivityService.js`. A second pass was performed to remove these files and lingering imports exhaustively.
